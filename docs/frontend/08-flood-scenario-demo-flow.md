...
위험 주차장을 선택하면 위험 상세의 지도 목적지도 선택한 주차장으로 바뀐다. 현재 데모는 이미 계산된 실제 OSM 도로 경로 중 위험 주차장으로 접근하는 구간을 역방향으로 잘라 사용한다. 따라서 상세 지도에는 현재 위치 마커에서 위험 주차장 `P` 마커까지 도로 형태의 경로가 보인다. 임의의 GPS 출발점에서 모든 주차장까지 다시 계산하는 기능은 백엔드 라우팅 API 연결 시 `frontend/src/lib/parkingDetailRoute.ts`를 교체한다.
