...
평상시 홈과 긴급 경고는 서로 다른 진입 흐름이다.

1. 평상시 `?view=map` — Figma `123:1958`: 실제 Kakao 지도에서 주변 주차장 선택
2. 선택 결과가 고위험이면 `?view=risk-detail` — Figma `119:1140`
3. 선택 결과가 저위험이면 `?view=safe-detail` — Figma `123:1743`
4. `?view=emergency` — Figma `90:675`: 비·경고 진동과 차량 이동 경고
5. 긴급 CTA는 선택 화면을 다시 열지 않는다. 안전 후보별 침수 회피 경로를 계산하고 도로거리가 가장 짧은 후보를 자동 선택한다.
6. 즉시 `?view=route` — Figma `244:3303`: 실제 Kakao 지도 위에 위험 폴리곤과 청록색 저위험 경로 표시

위험 상세와 안전 상세는 앞뒤로 이어지는 화면이 아니라 **주차장 침수 위험 판정 결과에 따른 상호 배타적 분기**다. 현재는 `frontend/src/lib/parkingRisk.ts`의 데모 어댑터가 시나리오에 등록된 저위험 후보 ID를 `safe`, 그 외 후보를 `danger`로 반환한다. 백엔드 계약이 정해지면 이 어댑터만 실제 API 호출로 교체한다.

위험 주차장을 선택하면 위험 상세의 지도 목적지도 선택한 주차장으로 바뀐다. 힌남노 데모는 `제철복지회관 임시주차장` 하나를 명시적인 Safe 후보로 사용한다. 오천읍 구정길의 사용자 시연 위치에서 저장한 내 차 위치까지, Safe 후보를 선택하면 저장한 내 차 위치에서 선택 주차장까지 `/api/flood-route`로 새로 계산한다. Python 백엔드는 합성 `CURRENT` 침수 폴리곤과 교차하는 OSM 도로 간선을 제거하고 다익스트라를 다시 실행한다. 따라서 상세 지도에는 각 단계의 실제 출발점·목적지, 계산에 사용된 위험 폴리곤과 우회 도로가 함께 보인다.

위험 상세 CTA도 긴급 CTA와 같은 최인접 안전 후보 탐색을 실행해 길찾기로 이동한다. 안전 상세 CTA는 사용자가 선택한 안전 후보까지의 경로로 이동한다.
...
- 402×874 브라우저에서 `평상시 지도 → 위험 또는 안전 상세` 분기와 `긴급 → 자동 안전 후보 선택 → 길찾기` 전환 성공
...
- [Figma 우회 길찾기 `244:3303`](https://www.figma.com/design/rq2THpj29lq6OhqCq6xcAw/-Junction--Uneducated-Kids?node-id=244-3303&m=dev)
