...
위험 주차장을 선택하면 위험 상세의 지도 목적지도 선택한 주차장으로 바뀐다. 힌남노 데모는 오천읍 구정길의 사용자 시연 위치에서 저장한 내 차 위치까지, 안전 후보를 선택하면 저장한 내 차 위치에서 선택 주차장까지 `/api/flood-route`로 새로 계산한다. Python 백엔드는 합성 `CURRENT` 침수 폴리곤과 교차하는 OSM 도로 간선을 제거하고 다익스트라를 다시 실행한다. 따라서 상세 지도에는 각 단계의 실제 출발점·목적지, 계산에 사용된 위험 폴리곤과 우회 도로가 함께 보인다.
