...
목록의 `가까운 순` 오른쪽에 있던 `Kakao`/`공공데이터` 출처 문구는 제거했다. 데이터 출처 메타데이터는 삭제하지 않고 내부 검색 결과와 조사 문서에 유지한다. 목록과 클릭 후 상세 모두 좌표 반경 120m의 가장 가까운 Kakao 실제 로드뷰를 표시한다. 작은 목록 칸에서도 현장 전경이 보이도록 로드뷰를 2배 크기로 렌더링한 뒤 축소한다. 로드뷰가 없거나 SDK가 실패할 때만 로컬 이미지를 폴백으로 사용한다. 힌남노 당시 침수 보도 사진은 제품 화면에서 제거했으며 시연 데이터와 장소 전경을 분리했다.
