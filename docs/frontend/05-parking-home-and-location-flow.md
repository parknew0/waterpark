...
목록의 `가까운 순` 오른쪽에 있던 `Kakao`/`공공데이터` 출처 문구는 제거했다. 데이터 출처 메타데이터는 삭제하지 않고 내부 검색 결과와 조사 문서에 유지한다. `ParkingPlace.imageUrl`이 있으면 해당 장소의 확인된 사진을 목록과 상세에서 함께 사용한다. 사진 URL이 없는 장소는 목록과 클릭 후 상세 모두 좌표 반경 120m의 가장 가까운 Kakao 실제 로드뷰를 표시한다. 작은 목록 칸에서도 현장 전경이 보이도록 로드뷰를 2배 크기로 렌더링한 뒤 축소한다. 로드뷰가 없거나 SDK가 실패할 때만 로컬 이미지를 폴백으로 사용한다. 힌남노 재연의 우방신세계타운 1차에는 확인한 2022년 현장 보도 이미지 URL을 연결했으며 외부 저작물의 배포 사용은 별도 허락 확인이 필요하다.
