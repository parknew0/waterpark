...
최신 기본 흐름에서는 `Next`가 지도 홈으로 바로 이동하면서 `navigator.geolocation`을 실행한다. 기존 위치 권한 설명 화면은 `?view=consent`에서 접근 가능한 보조 경로로 남겼다. 위치를 거부하거나 확인할 수 없으면 기본 경북 중심의 공공주차장을 보여주고 주소 검색을 사용할 수 있다.
