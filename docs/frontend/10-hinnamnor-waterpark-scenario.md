...
→ Current parking risk
→ Lower-risk parking candidate
→ OSM road route
```
...
재연 모드에서도 Splash·온보딩·지도·바텀시트·긴급 경고·상세·길찾기는 기존 Waterpark 컴포넌트를 그대로 사용한다. 차량 위치 확정 CTA를 누르면 홈 화면을 1.8초 보여준 뒤 긴급 경고가 자동으로 열린다.
