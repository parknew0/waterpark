...
`Warning` 상세는 안전 후보로 가는 경로를 재사용하지 않는다. 선택한 우방신세계타운 1차 지하주차장을 목적지로 삼고, 저장된 실제 OSM 경로의 접근 구간을 반대로 사용해 현재 위치에서 위험 주차장까지 가는 길을 지도에 그린다. 이 구간은 발표용 고정 재연이며 임의 GPS 재탐색은 백엔드 경로 API 연결 대상이다.
...
- 위험 상세의 접근 경로는 힌남노 플로우 재연용 고정 OSM 구간이다. 당시 실제 차량 이동 기록이나 실시간 도로 통제 기록이 아니다.
