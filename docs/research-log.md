...
### 2026-08-25 — 기상청 HSR 레이더 사건 수집 완료

- `FACT`: 기상청 API허브 HSR 이진 격자에서 우선순위 침수 사건 9개(`20191002`, `20200801`, `20200802`, `20200803`, `20200807`, `20200808`, `20210705`, `20220809`, `20220814`)를 먼저 추가 수집했다. 이어 침수 기록 200건 이상 후보 중 남아 있던 `20181005`, `20200730`, `20200903`도 추가 수집해, 기존 7개와 합쳐 `data/interim/radar/events/`에 사건 파일 19개가 있다.
- `FACT`: 침수 시각이 확인된 최초 추가 사건 8개와 마지막 추가 사건 3개는 24시간 창을 각각 39프레임으로, 시각을 확인하지 못한 `20220814`는 사건일 전체를 포함하는 창을 63프레임으로 저장했다. 각 파일은 프레임별 대표 구간 `span_min`을 함께 저장한다.
- `FACT`: 19개 파일 모두 `series` 행 수와 `stamps`·`span_min` 길이가 일치하고 시각 중복이 없다. `series` 결측률은 0%이며, 파일 안의 `lon`·`lat`·`kind`·`owner` 길이는 강수 열 수와 모두 일치한다.
- `FACT`: 우선순위 9개 재개 실행은 동시 연결 2개, 요청 시작 최소 간격 8초로 수행했고 약 1.4~1.7MB/s에서 완료됐다. 일부 단일 프레임 읽기 타임아웃은 두 번째 시도에 복구됐으며, 해당 최종 로그에는 접속 차단·미저장·키 소진이 없다.
- `FACT`: 마지막 3개도 같은 동시 연결 수와 요청 간격으로 117/117프레임을 수신했다. 첫 번째 키가 일일 한도에 도달한 뒤 남은 키로 자동 전환했으며, 접속 차단이나 미저장 없이 세 파일을 모두 완성했다.
- `FACT`: 수집기는 한 사건의 모든 요청을 미리 예약하지 않고 워커 수만큼만 유지하도록 수정했다. 연속 연결 실패 시 아직 시작하지 않은 요청을 취소하므로, 접속 차단 뒤 예약 요청이 계속 실행되는 문제를 막는다.
- `LIMITATION`: 기존 `20220808`·`20230715` 파일은 44,872개 지점과 288프레임, 나머지는 45,782개 지점과 39·63·78프레임이다. 다운스트림 결합은 외부 지점 목록을 재사용하지 않고 각 파일이 저장한 좌표·종류·사건 소유자를 읽어야 한다.
- 출처: [기상청 API허브 레이더 자료](https://apihub.kma.go.kr/apiList.do?seqApi=5), `data/interim/radar/collector.log`, `data/interim/radar/events/`
- 확인일: 2026-08-25

## 결정 로그

### 2026-08-23 — 평상시 주차장 마커의 안전 상세·실제 경로 연결

- `FACT`: 기존 일반 모드는 `safeParkingCandidates`가 비어 있고 정적 대피 경로가 아직 로드되지 않으면 마커 클릭 핸들러가 조기 종료돼 `P`를 눌러도 상세 화면으로 이동하지 않았다.
- `DECISION`: 일반 모드에서는 현재 지도 검색 결과 중 저장한 내 차 위치를 제외한 주차장을 판정 후보로 사용한다.
- `DECISION`: 저장한 내 차가 있는 지도에서 `P`를 클릭하면 정적 경로 로드 여부와 관계없이 저장 위치와 선택 주차장 좌표로 실제 도로 경로를 계산한다. 현재 기본 판정 어댑터의 후보는 Safe로 분기해 Figma `123:1743` 상세에 지도·경로·거리·시간을 표시한다.
- `FACT`: 힌남노 Python 도로 그래프는 오천읍 시연 범위로 제한돼 평상시 효자동 좌표를 3km 이내 도로 노드에 스냅할 수 없다.
- `DECISION`: 위험 폴리곤이 없는 평상시 Safe 상세은 OSRM/OpenStreetMap의 전국 실제 도로 형상을 사용한다. 힌남노 모드는 계속 `/api/flood-route`의 차단 간선 다익스트라를 사용하며 두 경로를 섞지 않는다.
- `LIMITATION`: 평상시 Safe 판정은 실제 운영 예측 API가 아니라 `parkingRisk.ts`의 데모 어댑터 결과다. 운영 전에는 선택 좌표의 `/api/flood-risk` 응답과 주차장 건물 매핑으로 교체해야 한다.
- `DECISION`: 힌남노 안전 후보의 동적 경로 요청이 일시 실패해도 검증된 번들 경로가 있으면 긴급 화면에 실패 문구를 남기지 않고 번들 경로로 복구한다. 사용자가 CTA를 누른 시점에도 같은 폴백을 적용한다.
- `DECISION`: 힌남노 시연에서는 `제철복지회관 임시주차장`(`parking:d14f3c7979a80a1bda68`) 하나만 명시적인 Safe 후보로 고정한다. 우방신세계타운 지하주차장과 그 밖의 주변 후보는 Warning 분기로 남겨 긴급 CTA의 목적지가 사라지지 않게 한다.
- `DECISION`: 820px 이하의 짧은 화면에서는 상세 지도 높이를 240px로 줄이고 시트·근거 카드·CTA 위치를 함께 재배치해 버튼이 근거 목록을 덮지 않게 한다. 761px 화면이 기존 760px 조건을 1px 벗어나 겹친 사례를 반영한 범위다.
- `DECISION`: 위험·안전 판정 상세의 경로 지도에서는 현재 위치를 원형 점만 표시하고 방위 삼각형은 숨긴다. 일반 지도와 최종 길찾기의 이동 방향 표시는 그대로 유지한다.
- `DECISION`: 실제 휴대전화에서는 짧은 viewport 때문에 문서 스크롤이 필요할 수 있으므로 스크롤 동작은 유지한다. 대신 `html`, `body`, `#root`의 WebKit·Firefox 스크롤바만 숨겨 iPhone의 세로 스크롤 인디케이터가 UI 위에 노출되지 않게 한다.
- 출처: [Figma 안전 장소 상세 `123:1743`](https://www.figma.com/design/rq2THpj29lq6OhqCq6xcAw/-Junction--Uneducated-Kids?node-id=123-1743&m=dev)
- 확인일: 2026-08-23
