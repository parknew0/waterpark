...
- `FACT`: 기상청 API허브 HSR 이진 격자에서 침수 사건 9개(`20191002`, `20200801`, `20200802`, `20200803`, `20200807`, `20200808`, `20210705`, `20220809`, `20220814`)를 추가 수집했다. 기존 7개와 합쳐 `data/interim/radar/events/`에 사건 파일 16개가 있다.
- `FACT`: 침수 시각이 확인된 신규 사건 8개는 24시간 창을 39프레임으로, 시각을 확인하지 못한 `20220814`는 사건일 전체를 포함하는 창을 63프레임으로 저장했다. 각 파일은 프레임별 대표 구간 `span_min`을 함께 저장한다.
- `FACT`: 16개 파일 모두 `series` 행 수와 `stamps`·`span_min` 길이가 일치하고 시각 중복이 없다. `series` 결측률은 0%이며, 파일 안의 `lon`·`lat`·`kind`·`owner` 길이는 강수 열 수와 모두 일치한다.
- `FACT`: 최종 재개 실행은 동시 연결 2개, 요청 시작 최소 간격 8초로 수행했고 약 1.4~1.7MB/s에서 완료됐다. 일부 단일 프레임 읽기 타임아웃은 두 번째 시도에 복구됐으며, 최종 로그에는 접속 차단·미저장·키 소진이 없다.
